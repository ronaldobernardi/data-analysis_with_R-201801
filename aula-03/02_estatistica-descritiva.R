...
subset_com_ano %>%
  summarise(tempo_medio_ano = mean(year(today()) - ano_ingresso))
## Determine o tempo médio de trabalho em anos, por UF
subset_com_ano %>%
  group_by(UF_EXERCICIO) %>%
  summarise(tempo_medio_ano = mean(year(today()) - ano_ingresso)) %>%
  ungroup() %>%
  arrange(desc(tempo_medio_ano))
## Determine a média salarial por ano de ingresso
subset_com_ano %>%
  group_by(ano_ingresso) %>%
  summarise(salario_medio_ano = mean(REMUNERACAO_REAIS)) %>%
  ungroup() %>%
  arrange(desc(salario_medio_ano))
=======
...
#' Adicione uma nova variável determinando, para cada UF, se a média é maior ou menor que a mediana.
#' Ao final, exiba a quantidade de UFs onde a mediana foi maior que a média.
#' 
## ------------------------------------------------------------------------
subset_salarios %>%
  group_by(UF_EXERCICIO) %>%
...
#' __Atividade II__
#' 
#' Qual sua justificativa para a quantidade de casos onde a mediana foi maior que a média? 
#' Dica: Observe o gráfico que mostra a média e a mediana. Há cauda longa? Em qual direção?
#' 
#' bla bla bla0
#' 
#' >> FIM DA ATIVIDADE
